...
const ProviderConsumersOnProviderComponent = () => {
    ...
    const handleGenerateInvoices = async () => {
        subscriptions.map(async (subscription) => {
            try {
                const service = await request("GET", `http://localhost:8080/api/${subscription.facility.type.toLowerCase()}/by-facilityId/${subscription.facility.facilityId}`);
                ...
                if (service.data.facility.type === "MOBILESERVICE" || service.data.facility.type === "INTERNET") {
                    ...
                    const last2Indexes = await request("GET", `http://localhost:8080/api/index/by-subscription/${subscription.subscriptionId}`);
