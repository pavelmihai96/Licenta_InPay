...
const ProviderConsumersOnFacilityComponent = () => {
    ...
    const fetchTableData = async (facilityId) => {
        ...
        try {
            const f = await request("GET", `http://localhost:8080/api/facility/${facilityId}`);
            setFacility(f.data);

            const s = await request("GET", `http://localhost:8080/api/subscription/by-ids/${facilityId}`);
...
export default ProviderConsumersOnFacilityComponent;